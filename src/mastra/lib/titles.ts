/**
 * Manual `TITRE` annotations, end to end (issue #7, ADR 0004/0005/0006).
 *
 * A `TITRE` annotation is an explicit editing intention added during cleanup
 * review — never a rewrite of the transcript. It anchors to a `Segment`
 * (`segments.ts`), the same finest-grained unit scenes place against, rather
 * than to a "section": sections are the structural analysis's concept
 * (ADR 0005) and don't exist yet at this point in the PRD's stack of issues.
 *
...
/**
 * Projects a rendered title annotation to the same `OverlayScene` shape a
 * B-roll scene is compositied from (`fcpxml.ts`). Titles then get placed and
 ...
 */
export function titleToOverlayScene(annotation: TitleAnnotation): OverlayScene {
  if (annotation.exportPath === null) {
    throw new Error(
      `Title annotation ${annotation.id} has no exportPath yet — render it before compositing.`
    )
  }
